...
void scheduler(void){
  ...
  for(;;){
    ...
    if(even_check == 0){ //there's no even_number pid process(FCFS)
        for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
            if(p->state != RUNNABLE)
                continue;
            if((p->pid % 2) == 0)
    ...
    else{// there are even_number pid process(RoundRobin)
        for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
            if(p->state != RUNNABLE)
                continue;
            if((p->pid % 2) == 1)
...
void scheduler(void)
{
  ...
  for(;;){
    ...
    acquire(&ptable.lock);
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
        if(p->queue_level == search_level && p->state == RUNNABLE){
            isFind = 1;
        }
    }

    if(isFind == 1){ //search_level_queue process is found
        // find highest priority process in same level
        struct proc* highest = 0;
        for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
            if(p->state == RUNNABLE && p->queue_level == search_level){
                if(highest == 0){
                    highest = p;
                }
                else{ // highest is not null
        ...
        //did find highest priority process in same level and operate
        total_ticks++;
        c->proc = highest;
        switchuvm(highest);
        ...
        if(highest->ticks >= (highest->queue_level)*2 + 4){
            highest->queue_level++;
            highest->ticks = 0;
        }
    ...
    else{ //in search_level_queue there's no runnable process
        search_level++;
        if(search_level == MLFQ_K){
            priorityboosting();
            search_level = 0;
        }
    }
...
int setpriority(int pid, int priority){
  ...
  if(priority >= 0 && priority <= 10){
    acquire(&ptable.lock);
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
        if(pid == p->pid && myproc()->pid == p->parent->pid){
            p->priority = priority;
            release(&ptable.lock);
            return 0;
        }
